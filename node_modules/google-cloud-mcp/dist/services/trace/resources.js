 ...
 */
export function registerTraceResources(server) {
    // Resource to get a specific trace by ID
    server.resource('trace-by-id', new ResourceTemplate('gcp-trace://{projectId}/traces/{traceId}', { list: undefined }), async (uri, { projectId, traceId }, context) => {
        try {
    ...
    server.resource('trace-logs', new ResourceTemplate('gcp-trace://{projectId}/traces/{traceId}/logs', { list: undefined }), async (uri, { projectId, traceId }, context) => {
        try {
            // Use provided project ID or get the default one
            const actualProjectId = projectId || await getProjectId();
            // Validate trace ID format (hex string)
...
Filter used: ${traceFilter}`
            ...
            for (const entry of entries) {
                ...
                const message = logEntry.jsonPayload && typeof logEntry.jsonPayload === 'object' && 'message' in logEntry.jsonPayload
            ...
            // Error handling
            throw new GcpMcpError(`Failed to fetch logs for trace: ${error.message}`, error.code || 'UNKNOWN', error.statusCode || 500);
        }
    });
    // Resource to list recent failed traces
    server.resource('recent-failed-traces', new ResourceTemplate('gcp-trace://{projectId}/recent-failed', { list: undefined }), async (uri, { projectId }, context) => {
        try {
            // Use provided project ID or get the default one
            const actualProjectId = projectId || await getProjectId();
            ...
            if (!response.ok) {
                ...
                throw new GcpMcpError(`Failed to fetch failed traces: ${errorText}`, 'FAILED_PRECONDITION', response.status);
            }
            ...
            throw new GcpMcpError(`Failed to fetch failed traces: ${error.message}`, error.code || 'UNKNOWN', error.statusCode || 500);
        }
    });
}
/**
